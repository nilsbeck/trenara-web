...
- **State Validation:** Before any mutation (e.g., borrowing a book), query the database _inside_ the Server Action to verify the current state (e.g., `is_available === true`). Do not rely on client-side state.
