...
export function createCalendarStore(initialDate: Date): CalendarStore {
	...
	let selectedDateString = $derived(selectedDate ? formatDateString(selectedDate) : null);

	let filteredTrainings = $derived(
		schedule?.trainings.filter((t) => t.day_long === selectedDateString) ?? []
	);
...
export function createErrorBoundary(retryFn?: () => Promise<void>): ErrorBoundary {
	let error = $state<Error | null>(null);
