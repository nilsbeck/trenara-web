...
async getMessages(threadId: number): Promise<Message[]> {
    const response = await apiClient.getAxios().get<Message[]>(`/api/threads/${threadId}/messages`);
    return response.data;
},

async sendMessage(threadId: number, content: string): Promise<Message> {
    const response = await apiClient.getAxios().post<Message>(`/api/threads/${threadId}/messages`, { content });
    return response.data;
},
